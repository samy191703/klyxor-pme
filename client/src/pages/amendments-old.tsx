...
export default function Amendments() {
  ...
  const { data: contracts = [] } = useQuery<any[]>({
    ...
    enabled: showNewAmendmentDialog // Only fetch when dialog is open
  });

  // Mutation pour créer un avenant
  const createAmendmentMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: "Avenant créé",
        description: "L'avenant a été créé et envoyé pour validation"
      });
  ...
  // Get contract details for amendments
  const getContractDetails = (contractId: string) => {
    const contract = contracts.find((c: any) => c.id === contractId);
    return contract ? { number: contract.number, title: contract.title } : { number: "", title: "" };
  };

  // Enhanced amendments with contract details
  const amendmentsWithContractDetails = amendments.map(amendment => {
    const contractDetails = getContractDetails(amendment.contractId);
    return {
      ...amendment,
  ...
  // Placeholder for filtered amendments (previously mock data)
  const filteredAmendments = amendmentsWithContractDetails.filter(amendment => {
    // Apply filters
                ...
                <Card>
                  <CardContent className="p-4">
                    ...
                    <div className="text-sm space-y-1">
                      <div>N° Contrat: {newAmendment.contractNumber}</div>
                      <div>Intitulé: {newAmendment.contractTitle}</div>
                      <div>Statut: <Badge variant="secondary">Actif</Badge></div>
                      {(() => {
                        const contract = contracts.find((c: any) => c.id === newAmendment.contractId);
                        return contract ? (
                          <>
                            <div>Dates: {new Date(contract.startDate).toLocaleDateString('fr-FR')} - {new Date(contract.endDate).toLocaleDateString('fr-FR')}</div>
                            <div>Montant: {contract.amount} {contract.currency}</div>
                          </>
                        ) : null;
                      })()}
                    </div>
              ...
              <div className="flex gap-2">
                ...
                {currentStep < 4 ? (
                  ...
                  <Button 
                    onClick={() => {
                      // Créer l'avenant
                      if (!newAmendment.contractId || !newAmendment.reason) {
                        toast({
                          title: "Erreur",
                      ...
                      const amendmentData = {
                        ...
                        contractNumber: newAmendment.contractNumber,
                        contractTitle: newAmendment.contractTitle,
                        modificationType: newAmendment.modificationType || 'clause',
                        reason: newAmendment.reason,
                        effectiveDate: newAmendment.effectiveDate || new Date().toISOString().split('T')[0],
                        affectedFields: 'Modification contractuelle',
                        beforeValue: '-',
                        afterValue: '-',
                        status: 'to_validate',
      ...
      <Dialog open={showValidationModal} onOpenChange={setShowValidationModal}>
        <DialogContent className="w-[95vw] max-w-[500px] max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Examinez les détails de l'avenant et prenez une décision de validation
            </DialogDescription>
          </DialogHeader>
