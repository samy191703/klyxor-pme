...
export default function Workflows() {
  ...
  const completedWorkflows = workflowInstances.filter(
    (w) => w.status === "completed"
  );
  const failedWorkflows = workflowInstances.filter(
  ...
  const startModificationMutation = useMutation({
    ...
    onSuccess: () => {
      toast({
        title: "Workflow démarré",
        description:
          "Le processus de modification a été initié avec succès.",
      });
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-base">
              {workflow.entityType === "termination" &&
                "Résiliation de contrat"}
              {workflow.entityType === "modification" &&
      ...
      <CardContent>
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>
              Date: {new Date(workflow.startedAt).toLocaleDateString("fr-FR")}
            </span>
          </div>
          {workflow.status === "in_progress" &&
            workflow.currentStep !== undefined && (
              <div className="mt-3">
                ...
                <Progress
                  value={(workflow.currentStep + 1) * 25}
                  className="h-2"
                />
              </div>
            )}
    ...
    return (
      <div className="flex gap-2">
        <Button
          ...
          data-testid="button-start-termination"
        ...
        <Button
          ...
          variant="outline"
          data-testid="button-start-modification"
          className={cn(
            "gap-2 text-xs",
            isDark
  ...
  return (
    <>
      <KlyxorPageLayout
        ...
        subtitle="Orchestration des processus métier et suivi des workflows."
      ...
      >
        {(theme) => {
          ...
          return (
            <div className="max-w-7xl mx-auto space-y-4" data-testid="validation-main">
              {/* Statistiques / Hero */}
              <Card className={heroCardClass}>
                <CardContent className="grid gap-4 px-4 py-4 md:grid-cols-4">
                  ...
                  <StatCard
                    title="Temps moyen"
                    value={2.5}
                    description="Jours par workflow"
                  />
                </CardContent>
              </Card>
      ...
      >
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Initiez le processus de résiliation d&apos;un contrat avec toutes
              les validations nécessaires.
            </DialogDescription>
          </DialogHeader>
          <Form {...terminationForm}>
            <form
              onSubmit={terminationForm.handleSubmit(onSubmitTermination)}
              className="space-y-4"
            >
              <FormField
              ...
              <DialogFooter>
                ...
                <Button
                  type="submit"
                  disabled={startTerminationMutation.isPending}
                  data-testid="button-submit-termination"
                >
                  {startTerminationMutation.isPending && (
      ...
      >
        <DialogContent className="max-w-2xl">
          ...
          <Form {...modificationForm}>
            <form
              onSubmit={modificationForm.handleSubmit(onSubmitModification)}
              className="space-y-4"
            >
              <FormField
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>
                      Cette justification sera incluse dans le workflow de
                      validation.
                    </FormDescription>
                  </FormItem>
                )}
              ...
              <DialogFooter>
                ...
                <Button
                  type="submit"
                  disabled={startModificationMutation.isPending}
                  data-testid="button-submit-modification"
                >
                  {startModificationMutation.isPending && (
                    <RefreshCw className="h-4 w-4 mr-2 animate-spin" />
                  )}
                  Démarrer le workflow
                </Button>
