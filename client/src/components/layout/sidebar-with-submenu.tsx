...
export default function SidebarWithSubmenu() {
  ...
  // IMPORTANT: en DEV, tu sembles être "dev.admin" (cf capture). On le traite comme admin.
  const isAdmin = userRole === "admin" || userRole === "dev.admin";

  // Arborescence validée (Klyxor V1)
  const tree: NavItem[] = useMemo(
    () => [
...

      {
        ...
        icon: FileText,
        children: [
          { key: "contracts.gestion", label: "Gestion", href: "/contracts", icon: FileText },
          { key: "contracts.amendments", label: "Avenants", href: "/amendments", icon: Edit },
          { key: "contracts.terminations", label: "Résiliations", href: "/terminations", icon: XCircle },
          {
            ...
            icon: CreditCard,
            children: [
              { key: "billing.invoices", label: "Factures", href: "/invoices", icon: FileCheck },
              { key: "billing.plans", label: "Plan de facturation", href: "/billing-plans", icon: Calendar },
            ],
          },
        ],
      },

      {
        ...
        icon: GitBranch,
        children: [
          { key: "validation.workflows", label: "Workflows", href: "/validation", icon: GitBranch },
          { key: "validation.deadlines", label: "Échéances & rappels", href: "/admin/deadlines", icon: Calendar },
        ],
      },

      {
        ...
        icon: Calculator,
        children: [
          { key: "indexation.indices", label: "Indices", href: "/admin/indexations", icon: BarChart },
          { key: "indexation.history", label: "Historique", href: "/indexations", icon: TrendingUp },
          { key: "indexation.reports", label: "Rapports", href: "/indexation-dashboard", icon: FileText },
        ],
      },

      {
        ...
        children: [
          ...
          { key: "docs.import", label: "Import de fichier", href: "/imports", icon: Database },
...

      {
        ...
        icon: Shield,
        children: [
          { key: "admin.security", label: "Sécurité & Conformité", href: "/admin/security", icon: Shield },
        ],
      },
    ],
    [],
  ...
  /**
   * Permissions:
   * - Admin : voit tout (pas de filtre hasPermission)
   * - Non-admin : on garde les items visibles mais grisés si non autorisés
   */
  const filteredTree: NavItem[] = useMemo(() => {
    if (isAdmin) return tree;

    const filterNode = (n: NavItem): NavItem | null => {
      // Admin menu caché pour non-admin
      if (n.label === "Administration") return null;

      let disabled = false;

      if (n.href) {
        const checkHref = pathOnly(n.href);
        if (!hasPermission(checkHref)) disabled = true;
      }
      ...
      if (n.children?.length) {
        ...
        const allKidsDisabled = kids.every((k) => !!k.disabled);
        return { ...n, children: kids, disabled: disabled || allKidsDisabled };
      }
  ...
  const labelCls = "truncate text-[13px] text-slate-800";
      ...
      <div key={n.key} className="space-y-1">
        ...
        {expanded ? (
          <div className="space-y-1">
            {n.children.map((c) =>
              renderNode(c, Math.min(level + 1, 2) as 1 | 2),
            )}
          </div>
        ) : null}
      </div>
  ...
  return (
    ...
    >
      ...
      <div className="px-5 py-5 border-b border-slate-200 bg-white">
        <a href="/" className="flex items-center gap-3">
          ...
          <div className="leading-tight">
            ...
            <div className="text-[12px] text-slate-500">Contract Management</div>
          </div>
        </a>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto px-2 py-3 bg-white">
        <SectionLabel />
        <div className="space-y-2 px-1">{filteredTree.map((n) => renderNode(n, 0))}</div>
      </nav>
