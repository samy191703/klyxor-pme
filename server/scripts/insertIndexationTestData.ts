...
async function insertTestData() {
  ...
  try {
    ...
    const formula1 = await db.insert(indexationFormulas).values({
    ...
    const formula2 = await db.insert(indexationFormulas).values({
      name: "Formule Pondérée ICHT/FMOA",
      expression: "P = P₀ × (0,15 + 0,55 × (ICHTREV / ICHT₀) + 0,3 × (FMOAREV / FMOA₀))",
      variables: ["ICHT", "FMOA"],
    ...
    // Formule 3: CPI
    const formula3 = await db.insert(indexationFormulas).values({
      name: "Formule CPI",
    ...
    await db.insert(contracts).values({
      ...
      indexationCurrentAmount: 52919.20,
      indexationIndices: {},
      indexationCap: 0,
    ...
    for (const date of indexDates) {
      ...
      await db.insert(indexValues).values({
        ...
        status: date < new Date("2024-07-01") ? "final" : date < new Date("2024-09-01") ? "revised" : "provisional",
        variation: variation,
