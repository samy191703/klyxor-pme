...
import pkg from "pg";
const { Pool } = pkg;
import { anyClientSchema } from "../client.model";



// Pool PostgreSQL dédié au module Clients
const clientsDbPool = new Pool({
  connectionString: process.env.DATABASE_URL,
});

// Nettoie les chaînes de caractères (trim) dans le payload
function trimStringFields<T extends Record<string, any>>(payload: T): T {
  const result: any = {};
...
export async function registerRoutes(
  ...
  // ========== CLIENTS - LISTE PAGINÉE (BRANCHÉE DB) ==========
  /**
   * GET /api/clients
   * Liste paginée des clients avec recherche + tri
   * Compatible avec ClientsListPage.tsx
   */
  app.get("/api/clients", isAuthenticated, async (req, res) => {
    try {
      const page =
        Math.max(parseInt((req.query.page as string) || "1", 10), 1) || 1;
      const pageSize = 20;

      const rawSearch = (req.query.search as string | undefined) || "";
      const search = rawSearch.trim();
      const typeClient = req.query.type_client as
        | "PRO"
        | "PARTICULIER"
        | undefined;
      const sortParam = (req.query.sort as string) || "nom";
      const sort: "nom" | "type" =
        sortParam === "type" ? "type" : ("nom" as const);

      const whereClauses: string[] = [];
      const params: any[] = [];

      // Filtre type_client
      if (typeClient === "PRO" || typeClient === "PARTICULIER") {
        params.push(typeClient);
        whereClauses.push(`type_client = $${params.length}`);
      }

      // Recherche sur raison_sociale / prénom / nom / email / siret
      if (search) {
        params.push(`%${search.toLowerCase()}%`);
        const idx = params.length;
        whereClauses.push(
          `(
            LOWER(COALESCE(raison_sociale, '')) LIKE $${idx}
            OR LOWER(COALESCE(prenom, '')) LIKE $${idx}
            OR LOWER(COALESCE(nom, '')) LIKE $${idx}
            OR LOWER(email) LIKE $${idx}
            OR COALESCE(siret, '') LIKE $${idx}
          )`
        );
      }
      ...
      const orderBy =
        sort === "type"
          ...
          : `ORDER BY raison_sociale NULLS LAST, nom NULLS LAST, prenom NULLS LAST`;

      const offset = (page - 1) * pageSize;

      // 1) Compter le total
      const countResult = await clientsDbPool.query(
        `SELECT COUNT(*)::int AS count FROM clients ${whereSql}`,
        params
      );
      ...
      const dataResult = await clientsDbPool.query(
        `
        SELECT
          id,
          type_client,
    ...
    /**
   * POST /api/clients
   * Création d’un client professionnel ou particulier.
   * Règles :
   *  - Validation Zod via anyClientSchema
   *  - Unicité email
   *  - Unicité SIRET pour PRO
   *  - Actif = true
   */
    ...
    try {
      ...
      // Validation via Zod (client.model.ts)
      const parsed = anyClientSchema.parse(payload);

      // Vérification email unique
      const emailCheck = await clientsDbPool.query(
      ...
      // Vérification SIRET unique (PRO uniquement)
      if (parsed.type_client === "PRO" && parsed.siret) {
        const siretCheck = await clientsDbPool.query(
        ...
        if (siretCheck.rows.length > 0) {
          return res.status(400).json({
            ...
            message:
              "Un client professionnel avec ce SIRET existe déjà.",
          });
        }
      }
      ...
      const insert = await clientsDbPool.query(
        `
        INSERT INTO clients (
          type_client,
          raison_sociale,
      ...
      try {
        console.error(
          "Error in POST /api/clients:",
          err?.message || err
        );
      } catch {
        console.error("Error in POST /api/clients: [unloggable error]");
      }
  ...
  app.get("/api/clients/:id", isAuthenticated, async (req, res) => {
    try {
      ...
      const result = await clientsDbPool.query(
        `
        SELECT
          id,
          type_client,
   ...
   */
    ...
    try {
      ...
      // Validation via Zod avec protection
      let parsed;
      try {
        parsed = anyClientSchema.parse(payload);
      } catch (err: any) {
      try {
        console.error(
          "Error in PUT /api/clients/:id:",
          err?.message || err
        );
      } catch {
        console.error("Error in PUT /api/clients/:id: [unloggable error]");
      }
      ...
      // Vérification SIRET unique (PRO uniquement, hors lui-même)
      if (parsed.type_client === "PRO" && parsed.siret) {
        const siretCheck = await clientsDbPool.query(
        ...
        if (siretCheck.rows.length > 0) {
          return res.status(400).json({
            error: "SIRET_ALREADY_EXISTS",
            message: "Un autre client professionnel avec ce SIRET existe déjà.",
          });

        /**
   * PATCH /api/clients/:id/active
   * Met à jour le statut actif / inactif d'un client.
   */
  app.patch("/api/clients/:id/active", isAuthenticated, async (req, res) => {
    try {
      const { id } = req.params;
      ...
      try {
        console.error(
          "Error in PATCH /api/clients/:id/active:",
          err?.message || err
        );
      } catch {
        console.error(
          "Error in PATCH /api/clients/:id/active: [unloggable error]"
        );
      }

      return res
        .status(500)
        .json({ error: "Failed to update client status" });
    }
  });
      ...
      const updateResult = await clientsDbPool.query(
        `
        UPDATE clients
        SET
          type_client = $1,
      ...
      `,
        [
          ...
          parsed.ville ?? null,
          parsed.pays ?? null,
          // pour l’instant on laisse toujours actif = true
          true,
          now,
          id,
        ]
   ...
   * Règle V1 : interdiction si des contrats actifs ou en validation sont liés.
   */
  app.delete("/api/clients/:id", isAuthenticated, async (req, res) => {
    try {
      const { id } = req.params;

      // 1) Vérifier s'il existe des contrats liés
      const contractsResult = await clientsDbPool.query(
        `
        SELECT COUNT(*)::int AS count
        FROM contracts
        WHERE client_id = $1
          AND status IN ('active', 'pending_validation', 'draft')
        `,
        [id]
      );

      const contractsCount = contractsResult.rows[0]?.count ?? 0;

      if (contractsCount > 0) {
        return res.status(400).json({
          error: "CLIENT_HAS_CONTRACTS",
          message:
            "Ce client ne peut pas être supprimé car des contrats actifs ou en cours de validation y sont associés.",
          contractsCount,
        });
      }

      // 2) Si aucun contrat bloquant, on peut supprimer
      const result = await clientsDbPool.query(
      ...
      console.error(
        "Error in DELETE /api/clients/:id:",
        err?.message || err
      );
 ...
 * Retourne tous les contrats liés à un client
 */
app.get("/api/clients/:id/contracts", isAuthenticated, async (req, res) => {
  try {
    const { id } = req.params;
    ...
    console.error("Error in GET /api/clients/:id/contracts:", err);
   ...
   * Endpoint simplifié pour les dropdowns (Contrats, etc.)
   * Retourne seulement : id, nom_affichage, type_client
   */
  app.get("/api/clients/select", isAuthenticated, async (req, res) => {
    try {
      const result = await clientsDbPool.query(
        `
        SELECT
        ...
        ORDER BY
          CASE
            WHEN type_client = 'PRO' THEN COALESCE(raison_sociale, '')
            ELSE COALESCE(prenom || ' ' || nom, '')
          END
      `
      );
      ...
      const data = result.rows.map((c) => {
        const nom_affichage =
          c.type_client === "PRO"
            ? c.raison_sociale || "(Sans raison sociale)"
      ...
      console.error("Error in GET /api/clients/select:", err);
      res.status(500).json({ error: "Failed to fetch clients select list" });
    }
  });
