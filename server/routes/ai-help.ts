...
function buildUserPrompt(page: string, section: string, userAction?: string, data?: any): string {
  ...
  if (data) {
    prompt += ` Données contextuelles disponibles : ${JSON.stringify(data, null, 2)}`;
  }
