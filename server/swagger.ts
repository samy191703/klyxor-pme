...
function makeSpec(
  ...
  return swaggerJsdoc({
    definition: {
      ...
      components: {
        securitySchemes: {
          // Descriptive only (Swagger can't read HttpOnly cookies)
          cookieAuth: {
            type: "apiKey",
            in: "cookie",
          ...
          // Optional: if your /login returns a JWT too, we auto-preauthorize this.
          bearerAuth: { type: "http", scheme: "bearer", bearerFormat: "JWT" },
        },
      },
      // Optional default: require cookieAuth unless an endpoint overrides it
      security: [{ cookieAuth: [] }],
    },
    apis: opts.apis,
  });
...
(function () {
  ...
  async function checkSession() {
    try {
      ...
      // expect { user: { username, ... } } or similar
  ...
  async function doLogin(username, password) {
    ...
    try {
      data = await res.json();
      // adjust: token field name on your API
      token = data && (data.token || data.accessToken || data.jwt || null);
      currentUser = (data && (data.user || data.profile)) || null;
    } catch (e) {
      // Some login endpoints may not return JSON; cookie still set.
    }

    // If we got a JWT and Swagger UI is ready, preauthorize bearer
    try {
      if (token && getUi() && getUi().preauthorizeApiKey) {
        getUi().preauthorizeApiKey('bearerAuth', token);
      }
    } catch (e) { /* ignore */ }
  ...
  async function doLogout() {
    ...
    // Clear any bearer preauth (keeps UI consistent)
    try {
      if (getUi() && getUi().authActions && getUi().authActions.logout) {
        getUi().authActions.logout(['bearerAuth']);
      }
    } catch (e) { /* ignore */ }
  ...
  // Inject a simple modal for username/password
  function showLoginModal() {
    ...
    form.onsubmit = async function(ev){
      ...
      try {
        await doLogin(u.value, p.value);
        document.body.removeChild(overlay);
        alert('Login successful');
      } catch (e) {
        errBox.textContent = (e && e.message) || 'Login failed';
      } finally {
  ...
  function patchTryItOutCredentials() {
    try {
      ...
      var cfg = window.ui.getConfigs && window.ui.getConfigs();
      if (!cfg) return;
      // ensure all requests send cookies
      window.ui.getConfigs().requestInterceptor = function (r) {
  ...
  // bootstrap
  renderTopbar();
  patchTryItOutCredentials();
  checkSession();

  // Keep UI patched if Swagger re-renders
  ...
  // 4) Swagger UI
  // Mount assets first, then serve UI with config pointing to JSON spec & custom UI
  app.use(
    route,
    swaggerUi.serve,
    swaggerUi.setup(undefined, {
      swaggerOptions: {
        ...
        // NOTE: we also inject credentials in custom JS; here is a backup
