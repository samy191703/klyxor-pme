...
import indexationRouter from "./routes/indexation.routes";
import indexationRoutesV2 from "./routes/indexationRoutes";
import { registerIndexationV3Routes } from "./routes/indexationV3Routes";
import codeSnippetsRouter from "./routes/codeSnippets";
 ...
 */
export async function registerRoutes(app: Express, keycloakEnabled: boolean = false): Promise<Server> {
  // ========== CONFIGURATION DE L'AUTHENTIFICATION ==========
  if (keycloakEnabled) {
    // Utiliser Keycloak OAuth2/OIDC
    console.log('🔐 Routes Keycloak OAuth2 activées');
    const keycloakRoutes = await import('./routes/authKeycloak');
    app.use('/api/auth/keycloak', keycloakRoutes.default);
    
    // Rediriger les anciennes routes vers Keycloak
    ...
    // ========== ROUTES D'AUTHENTIFICATION LOCALE (FALLBACK) ==========
    console.log('⚠️ Authentification locale activée (mode développement)');
    /**
     * Route de connexion - Authentifie les utilisateurs via Passport.js
     * Utilise la stratégie locale avec bcrypt pour la sécurité
     */
    ...
    /**
     * Route de déconnexion - Termine la session utilisateur
     */
    app.post("/api/auth/logout", (req, res) => {
      req.logout((err) => {
        if (err) {
     ...
     * Retourne l'état de connexion sans exposer le mot de passe
     */
    app.get("/api/auth/check", (req, res) => {
  ...
  } // Fin des routes d'authentification locale
  ...
  /**
   * Liste tous les contrats énergétiques ENGIE
   * Inclut électricité, gaz, PPA renouvelables, maintenance
   */
  ...
  app.post("/api/contracts", requirePermission('contracts', 'create'), async (req, res) => {
    try {
      const { ContractNumberGenerator } = await import("./services/contractNumberGenerator");
      const { validateContract } = await import("./validators/contractValidator");
      
      // Validation backend complète des données
      ...
      const contractData = {
        ...
        nextIndexationDate: req.body.nextIndexationDate ? new Date(req.body.nextIndexationDate) : undefined,
        createdBy: (req as any).user?.id || 'system',
        status: 'draft'
      };
      
      // Generate contract number automatically if not provided
      if (!contractData.number) {
        contractData.number = await ContractNumberGenerator.generateContractNumber(
          contractData.type || 'electricity',
  ...
  // Update contract
  app.put("/api/contracts/:id", isAuthenticated, requirePermission('contracts', 'update'), async (req, res) => {
    try {
      const contractData = {
        ...req.body,
  ...
  // Update contract (PATCH method)
  app.patch("/api/contracts/:id", isAuthenticated, requirePermission('contracts', 'update'), async (req, res) => {
    try {
      // Préparer les données pour la mise à jour
      const updateData: any = {};
      
      // Liste des champs qui ne sont pas des dates
      const nonDateFields = [
        'indexationFrequency', 'indexationFormula', 'indexationCap',
        'indexationThreshold', 'calculationMode', 'indexationFormulaId'
      ];
      
      // Copier les champs non-date qui sont présents
      ...
      // Gérer les dates spécifiquement (si présentes et non null)
      if (req.body.nextIndexationDate !== undefined) {
        updateData.nextIndexationDate = req.body.nextIndexationDate ? new Date(req.body.nextIndexationDate) : null;
      }
      if (req.body.indexTakingDate !== undefined) {
        updateData.indexTakingDate = req.body.indexTakingDate ? new Date(req.body.indexTakingDate) : null;
      }
      if (req.body.lastIndexationDate !== undefined) {
        updateData.lastIndexationDate = req.body.lastIndexationDate ? new Date(req.body.lastIndexationDate) : null;
      }
  ...
  app.delete("/api/contracts/:id", requirePermission('contracts', 'delete'), async (req, res) => {
    try {
      ...
      // Check if contract can be deleted (only draft or terminated contracts)
      if (!['draft', 'terminated'].includes(contract.status)) {
        return res.status(400).json({ 
          error: "Seuls les contrats en brouillon ou résiliés peuvent être supprimés" 
        });
      }
  ...
  app.get("/api/validation-requests", isAuthenticated, async (req, res) => {
    try {
      // Récupérer les vraies demandes depuis la base
      const validationRequests = await storage.getValidationRequests();
      res.json(validationRequests);
    } catch (error) {
  ...
  app.post("/api/validation-requests", isAuthenticated, async (req, res) => {
    try {
      const { type, title, contractRef, priority, description, amount, documents, dueDate } = req.body;
      ...
      const newRequest = {
        ...
        requestedBy: (req.user as any)?.email || "Current User",
        requestedAt: new Date(),
        dueDate: dueDate ? new Date(dueDate) : new Date(Date.now() + 7 * 24 * 60 * 60 * 1000),
        currentStep: 1,
        totalSteps: 3,
        validators: [
          { name: "Jean Martin", role: "Responsable juridique", status: "pending" },
          { name: "Sophie Laurent", role: "Directeur financier", status: "waiting" },
          { name: "Pierre Dubois", role: "Directeur général", status: "waiting" }
        ],
        description,
        amount,
        documents: documents || []
      ...
      res.json({ success: true, message: "Demande de validation créée", request: newRequest });
    } catch (error) {
  ...
  app.get("/api/validation-requests/:id/workflow", isAuthenticated, async (req, res) => {
    try {
      const { id } = req.params;
        ...
        steps: [
          {
            ...
            duration: "1 jour",
            comments: "Contrat conforme aux standards"
          },
          {
            step: 2,
          ...
          },
          {
            ...
            status: "pending",
            estimatedStart: new Date("2024-01-12"),
        ...
        history: [
          {
            action: "created",
            by: "Marie Dupont",
            at: new Date("2024-01-08"),
            details: "Demande de validation créée"
          },
          {
            action: "assigned",
            by: "System",
            at: new Date("2024-01-08"),
            details: "Assigné à Jean Martin pour revue juridique"
          },
          {
            action: "approved",
            by: "Jean Martin",
            at: new Date("2024-01-09"),
            details: "Revue juridique approuvée"
          },
          {
            action: "assigned",
            by: "System",
            at: new Date("2024-01-09"),
            details: "Assigné à Sophie Laurent pour validation financière"
          }
        ]
      };
  ...
  app.post("/api/validation-requests/:id/approve", requireValidator, async (req, res) => {
    try {
      const { id } = req.params;
  ...
  app.post("/api/validation-requests/:id/reject", requireValidator, async (req, res) => {
    try {
      const { id } = req.params;
      ...
      const deadlines = [
        {
          ...
          actions: ["Préparer proposition", "Revoir conditions", "Négocier tarifs"]
        },
        {
          id: "dl-002",
          type: "indexation",
      ...
      const reminders = [
        {
          ...
          contractRef: "CTR-2023-001",
          message: "Le contrat ENGIE Solutions Paris arrive à échéance dans 5 jours. Veuillez préparer la proposition de renouvellement.",
          severity: "high",
        ...
        },
        {
          ...
          contractRef: "CTR-2023-089",
          message: "La facture Q4 2023 est en retard de paiement. Montant: 85,000€. Action immédiate requise.",
          severity: "critical",
        ...
        },
        {
          ...
          contractRef: "CTR-2023-045",
          message: "L'indexation trimestrielle est prévue pour le 31/01. Merci de préparer les calculs.",
          severity: "medium",
        ...
        },
        {
          ...
          contractRef: "CTR-2023-156",
          message: "L'avenant pour le contrat de maintenance Lille nécessite votre validation avant le 18/01.",
          severity: "high",
  ...
  app.post("/api/notification-preferences", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const { alertService } = await import("./services/alertNotificationService");
  ...
  app.post("/api/alerts/:id/mark-read", isAuthenticated, async (req: any, res) => {
    try {
      const { id } = req.params;
      const userId = req.user?.id;
      if (!userId) {
  ...
  app.post("/api/alerts/mark-multiple-read", isAuthenticated, async (req: any, res) => {
    try {
  ...
  app.get("/api/contracts/:contractId/amendments", isAuthenticated, async (req, res) => {
    try {
  ...
  // Create amendment
  app.post("/api/amendments", requirePermission('amendments', 'create'), async (req, res) => {
    try {
      const amendmentData = {
        ...req.body,
        effectiveDate: req.body.effectiveDate ? new Date(req.body.effectiveDate) : undefined,
        requestedBy: req.body.requestedBy || (req.user as any)?.id || "cfb99f09-eb0f-45fc-82c4-8c468fe67e5c",
      };
  ...
  app.put("/api/amendments/:id", isAuthenticated, async (req, res) => {
    try {
      ...
      const amendment = await storage.updateAmendment(req.params.id, amendmentData);
      if (!amendment) {
        return res.status(404).json({ error: "Amendment not found" });
      }
  ...
  app.post("/api/indexations", isAuthenticated, requirePermission('indexations', 'create'), async (req, res) => {
    try {
  ...
  app.patch("/api/indexations/:id", isAuthenticated, requirePermission('indexations', 'validate'), async (req, res) => {
    try {
      const { id } = req.params;
      ...
      const updatedIndexation = await storage.updateIndexation(id, {
        status,
        appliedDate: status === 'validated' ? (appliedDate || new Date()) : undefined
      });
      ...
      if (status === 'validated') {
        const contract = await storage.getContract(updatedIndexation.contractId);
        if (contract) {
  ...
  app.post("/api/indexations/calculate", isAuthenticated, requirePermission('indexations', 'read'), async (req, res) => {
    try {
      const { contractId, indexationDate, indexTakingDate, testMode = true } = req.body;
      
      // Import du service de calcul
      const { IndexationCalculationEngine } = await import("./services/indexationCalculationEngine");
      const engine = new IndexationCalculationEngine();
      
      const result = await engine.calculate({
        contractId,
        indexationDate: new Date(indexationDate),
        indexTakingDate: indexTakingDate ? new Date(indexTakingDate) : undefined,
        testMode
      });
      ...
      res.status(500).json({ error: error.message || "Failed to calculate indexation" });
    }
  });
  ...
  app.get("/api/validation-assignments/:id", isAuthenticated, async (req, res) => {
    try {
      const { id } = req.params;
  ...
  app.put("/api/validation-assignments/:id", isAuthenticated, async (req, res) => {
    try {
      const { id } = req.params;
      const assignment = await storage.updateValidationAssignment(id, req.body);
  ...
  app.delete("/api/validation-assignments/:id", isAuthenticated, async (req, res) => {
    try {
      const { id } = req.params;
   ...
   */
  app.get("/api/indexation-frequencies/:id", isAuthenticated, async (req, res) => {
    try {
      const { id } = req.params;
   ...
   */
  app.put("/api/indexation-frequencies/:id", isAuthenticated, async (req, res) => {
    try {
      const { id } = req.params;
   ...
   */
  app.delete("/api/indexation-frequencies/:id", isAuthenticated, async (req, res) => {
    try {
      const { id } = req.params;
  ...
  // Configuration d'indexation par contrat
  app.put("/api/contracts/:id/indexation-config", isAuthenticated, async (req, res) => {
    try {
      const { id } = req.params;
      ...
      res.status(500).json({ error: "Failed to update indexation configuration" });
    }
  });
  ...
  app.post("/api/indexation/calculate", isAuthenticated, async (req, res) => {
    try {
      const { indexationEngine } = await import("./services/indexationCalculationEngine");
      ...
      res.status(500).json({ 
        error: error instanceof Error ? error.message : "Failed to calculate indexation" 
      });
    }
  });

  // Récupération des indices manquants
  app.get("/api/contracts/:id/missing-indices", isAuthenticated, async (req, res) => {
    try {
      const { indexationEngine } = await import("./services/indexationCalculationEngine");
      const { id } = req.params;
  ...
  app.get("/api/indexation/stats", isAuthenticated, async (req, res) => {
    try {
      ...
      const validatedThisMonth = proposals.filter(
        ...
        p.validatedAt && new Date(p.validatedAt) >= startOfMonth
      ).length;
      
      // Variation moyenne
      const validatedProposals = proposals.filter(p => p.status === "validated" && p.deltaPercent);
      const averageVariation = validatedProposals.length > 0
        ? validatedProposals.reduce((sum, p) => sum + Number(p.deltaPercent), 0) / validatedProposals.length
        : 0;
      
      // Prochaines indexations (30 jours)
      const thirtyDaysFromNow = new Date(now.getTime() + 30 * 24 * 60 * 60 * 1000);
      const nextIndexations = indexableContracts.filter(
        c => c.nextIndexationDate && 
        new Date(c.nextIndexationDate) <= thirtyDaysFromNow
      );
      
      res.json({
        totalContracts: indexableContracts.length,
        pendingIndexations: proposals.filter(p => p.status === "calculated" || p.status === "pending").length,
        validatedThisMonth,
  ...
  // Dashboard - Contrats avec indexation à venir
  app.get("/api/contracts/upcoming-indexations", isAuthenticated, async (req, res) => {
    try {
      const contracts = await storage.getContracts();
      const now = new Date();
      const thirtyDaysFromNow = new Date(now.getTime() + 30 * 24 * 60 * 60 * 1000);
  ...
  app.get("/api/index-values/latest", isAuthenticated, async (req, res) => {
    try {
      ...
      for (const value of indexValues) {
        if (!latestByCode.has(value.indexCode) || 
            new Date(value.period) > new Date(latestByCode.get(value.indexCode).period)) {
  ...
  app.post("/api/import/excel", isAuthenticated, async (req, res) => {
    try {
      ...
      await storage.createImportLog({
        ...
        errorReport: results.errors > 0 ? `Import de ${req.body.dataType || "contracts"}: ${results.errors} erreurs rencontrées` : null
      });
  ...
  app.post("/api/admin/contracts", async (req, res) => {
    try {
      const { ContractNumberGenerator } = await import("./services/contractNumberGenerator");
      const { BillingPlanGenerator } = await import("./services/billingPlanGenerator");
      const { validateContract } = await import("./validators/contractValidator");
  ...
  // Manual amount modification with justification
  app.post("/api/contracts/manual-modification", async (req, res) => {
    try {
      const {
        contractId,
        modificationType,
      ...
      if (!contractId || !justification || justification.length < 20) {
        return res.status(400).json({ 
          error: "Contract ID et justification détaillée (min 20 caractères) requis" 
        });
      }
      ...
      await storage.createAuditLog({
        ...
        details: {
          ...
          documentReference,
          approverEmail,
          percentageChange: ((newAmount - currentAmount) / currentAmount * 100).toFixed(2)
        },
        ipAddress: req.ip || '',
      ...
      const updatedContract = await storage.updateContract(contractId, {
        amount: newAmount,
        lastManualModification: new Date(),
        lastModificationJustification: justification,
        updatedAt: new Date()
      });
      ...
      console.error('Error in manual modification:', error);
      res.status(500).json({ error: "Failed to process manual modification" });
    }
  });
  ...
  app.get("/api/import-export/template/:type", async (req, res) => {
    try {
      ...
      const { type } = req.params as { type: 'contracts' | 'amendments' | 'indexations' };
      
      const buffer = ImportExportService.generateTemplate(type);
      
      res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
      res.setHeader('Content-Disposition', `attachment; filename=template_${type}_${new Date().toISOString().split('T')[0]}.xlsx`);
      res.send(buffer);
    } catch (error) {
  ...
  app.post("/api/import-export/export", async (req, res) => {
    try {
      ...
      res.setHeader('Content-Disposition', `attachment; filename=export_${entityType}_${new Date().toISOString().split('T')[0]}.${format}`);
      res.send(buffer);
    } catch (error) {
  ...
  app.get("/api/economic-indices/status", async (req, res) => {
    try {
      const { EconomicIndicesAutoFetcher } = await import("./services/economicIndicesAutoFetcher");
  ...
  app.post("/api/users", isAdmin, async (req, res) => {
    try {
      const { username, firstName, lastName, email, role, modules, status } = req.body;
      ...
      const newUser = await storage.createUser({
        ...
        email,
        role: role === 'admin' ? 'admin' : role === 'valideur' ? 'validator' : 'contract_manager'
      });
  ...
  app.get("/api/auth/permissions", isAuthenticated, async (req, res) => {
    ...
    res.json({
      ...
      permissions: {
        ...
        canApproveIndexations: hasPermission(userRole, 'indexations', 'validate'),
  ...
  app.get("/api/security/sensitive-access", requireAdmin, async (req, res) => {
    try {
      ...
      res.status(500).json({ error: "Failed to fetch sensitive data access logs" });
    }
  });
      ...
      const securityAlerts = [
        {
          ...
          severity: "critical",
          message: "5 tentatives de connexion échouées pour l'utilisateur 'jdupont'",
  ...
  app.post("/api/security/monitoring/check-lock", requireAdmin, async (req, res) => {
    try {
      const { username, ip } = req.body;
      const { securityMonitor } = await import('./services/securityMonitor');
      
      const isLocked = securityMonitor.isAccountLocked(username, ip);
      const remainingTime = securityMonitor.getRemainingLockTime(username, ip);
      const failedAttempts = securityMonitor.getFailedAttemptCount(username, ip);
  ...
  app.post("/api/security/monitoring/unlock", requireAdmin, async (req, res) => {
    try {
  ...
  app.get("/api/indexation/proposals", isAuthenticated, async (req, res) => {
    try {
      ...
      if (period) {
        ...
        proposals = proposals.filter(p => new Date(p.createdAt) >= cutoffDate);
      }
  ...
  app.get("/api/indexation/reports", isAuthenticated, async (req, res) => {
    try {
      ...
      const currentMonthIndexations = indexations.filter(i => 
        new Date(i.indexationDate) >= currentMonthStart
      );
      ...
      if (currentMonthIndexations.length > 0) {
        reports.push({
          ...
          statistics: {
            ...
            validated: currentMonthIndexations.filter(i => i.status === 'validated').length,
            rejected: currentMonthIndexations.filter(i => i.status === 'rejected').length,
            pending: currentMonthIndexations.filter(i => i.status === 'pending').length,
            totalVariation: currentMonthIndexations.reduce((sum, i) => 
              sum + (parseFloat(i.deltaPercentage || '0')), 0) / currentMonthIndexations.length || 0
          }
        });
      }
      ...
      if (lastMonthIndexations.length > 0) {
        reports.push({
          id: `RPT-${lastMonthStart.getFullYear()}-${lastMonthStart.getMonth() + 1}`,
          title: `Rapport mensuel - ${lastMonthStart.toLocaleString('fr-FR', { month: 'long', year: 'numeric' })}`,
          type: 'monthly',
          createdAt: lastMonthEnd,
          status: 'completed',
          statistics: {
            processed: lastMonthIndexations.length,
            validated: lastMonthIndexations.filter(i => i.status === 'validated').length,
            rejected: lastMonthIndexations.filter(i => i.status === 'rejected').length,
            pending: lastMonthIndexations.filter(i => i.status === 'pending').length,
            totalVariation: lastMonthIndexations.reduce((sum, i) => 
              sum + (parseFloat(i.deltaPercentage || '0')), 0) / lastMonthIndexations.length || 0
          }
        });
      }
  ...
  // Start scheduler
  app.post("/api/indexation/scheduler/start", isAuthenticated, async (req, res) => {
    try {
      schedulerState.isRunning = true;
      schedulerState.lastRun = new Date();
      schedulerState.nextRun = new Date(Date.now() + 24 * 60 * 60 * 1000); // Next day
      res.json({ success: true, message: 'Scheduler démarré', state: schedulerState });
    } catch (error) {
  ...
  // Stop scheduler
  app.post("/api/indexation/scheduler/stop", isAuthenticated, async (req, res) => {
    try {
      schedulerState.isRunning = false;
      schedulerState.nextRun = null;
      res.json({ success: true, message: 'Scheduler arrêté', state: schedulerState });
    } catch (error) {
  ...
  app.post("/api/indexation/scheduler/run", isAuthenticated, async (req, res) => {
    try {
      schedulerState.lastRun = new Date();
  ...
  app.post("/api/indexation/proposals/:id/validate", isAuthenticated, async (req, res) => {
    try {
      ...
      res.json({ 
        success: true, 
        message: `Proposition ${decision === 'validate' ? 'validée' : 'rejetée'}`,
        proposalId: id
      });
    } catch (error) {
      res.status(500).json({ error: "Failed to validate proposal" });
  ...
  // Generate report
  app.post("/api/indexation/reports/generate", isAuthenticated, async (req, res) => {
    try {
      ...
      if (period === 'month') {
        ...
        filteredIndexations = indexations.filter(i => 
          new Date(i.indexationDate) >= monthStart
        );
      } else if (period === 'quarter') {
        const quarterStart = new Date(now.getFullYear(), Math.floor(now.getMonth() / 3) * 3, 1);
        filteredIndexations = indexations.filter(i => 
          new Date(i.indexationDate) >= quarterStart
        );
      } else if (period === 'year') {
        const yearStart = new Date(now.getFullYear(), 0, 1);
        filteredIndexations = indexations.filter(i => 
          new Date(i.indexationDate) >= yearStart
        );
      }
      ...
      const report = {
        ...
        data: {
          ...
          validated: filteredIndexations.filter(i => i.status === 'validated').length,
          rejected: filteredIndexations.filter(i => i.status === 'rejected').length,
          pending: filteredIndexations.filter(i => i.status === 'pending').length,
          averageVariation: filteredIndexations.reduce((sum, i) => 
            sum + (parseFloat(i.deltaPercentage || '0')), 0) / filteredIndexations.length || 0,
          totalAmount: filteredIndexations.reduce((sum, i) => 
            sum + (parseFloat(i.newAmount || '0')), 0)
        }
      };
  ...
  app.get("/api/indexation-proposals/status/:status", isAuthenticated, async (req, res) => {
    try {
      const proposals = await storage.getIndexationProposalsByStatus(req.params.status);
      res.json(proposals);
    } catch (error) {
  ...
  app.get("/api/indexation-proposals/contract/:contractId", isAuthenticated, async (req, res) => {
    try {
      const proposals = await storage.getIndexationProposalsByContractId(req.params.contractId);
      res.json(proposals);
    } catch (error) {
  ...
  app.get("/api/indexation/eligible-contracts", isAuthenticated, async (req, res) => {
    try {
  ...
  app.post("/api/payment-proofs/generate", isAuthenticated, async (req, res) => {
    try {
      const { paymentId, invoiceIds, amount, currency, method, contractId, payer, bankReference } = req.body;
